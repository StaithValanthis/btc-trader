...
class MLService:
    ...
    MLService now trains two completely separate models:
      ...
      - A dedicated signal model trained as an ensemble with automated hyperparameter tuning
    ...
    Data is split 70/15/15 with walk-forward validation for cross-validation, and performance
    is evaluated using confusion matrix and weighted F1 score.
    """
    def __init__(
        self,
        lookback=120,
        ...
        # For trend model, composite features (3 features)
        self.trend_feature_cols = ["sma_diff", "adx", "dmi_diff"]
        # For signal model, initial feature set (9 features)
    ...
    async def train_trend_model(self):
        # Fallback if tuning is not enabled
        await self.tune_trend_model()
    ...
    async def tune_signal_model(self):
        ...
        # Automated Feature Selection: Using Random Forest to rank features
        ...
        if self.use_tuned_signal_model:
            self.signal_models = [best_model]  # Using tuned model for now (ensemble can be expanded)
    ...
    async def train_signal_ensemble(self, n_models=3):
        # Fallback training if tuning is not enabled.
    ...
    def predict_trend(self, recent_data: pd.DataFrame) -> str:
        if self.trend_model:
            data_seq = self._prepare_data_sequence(recent_data, self.actual_trend_cols or self.trend_feature_cols, resample_period="30min")
            if data_seq is None:
                return "Hold"
