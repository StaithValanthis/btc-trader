...
# Use MexcClient for live trading on MEXC.
...
def enhanced_get_market_regime(df, adx_period=14, base_threshold=25):
    """
    Determines market regime based on ADX, moving average crossover, and volatility checks.
    ...
    try:
        ...
        adx_indicator = ta.trend.ADXIndicator(
            high=df_clean["high"], low=df_clean["low"], close=df_clean["close"], window=adx_period
        )
        ...
        # Compute ATR percentile for adaptive threshold
        atr_indicator = ta.volatility.AverageTrueRange(
            high=df_clean["high"], low=df_clean["low"], close=df_clean["close"], window=14
        )
        ...
        # Adaptive threshold: reduce base_threshold as volatility decreases
...
class TradeService:
    """
    Primary trading service that:
      - Initializes the MLService for signal predictions.
      - Interacts with MEXC for placing orders and fetching positions.
      - Runs the main trading logic loop.
      - Aggregates candle data and updates stops.
    """
    ...
    async def initialize(self):
        ...
        Initialize trade service:
          1. Create MEXC client
          2. Fetch min order qty from instruments info
          3. Initialize ML service
        ...
        try:
            ...
            logger.info("Trade service initialized successfully for MEXC.")
        except Exception as e:
    ...
    async def _get_min_order_qty(self):
        """
        Attempt to retrieve min order quantity from MEXC's instruments info.
        If the response is None or invalid, we log an error and default to 1.0
        """
        if not self.min_qty:
            info = await asyncio.to_thread(
                self.session.get_instruments_info,
                symbol=Config.MEXC_CONFIG['symbol']
            )

            if not info:
                # MEXC returned None or all retries exhausted
                logger.critical("Failed to get instrument info (None). Setting min_qty=1.0")
                self.min_qty = 1.0
                return

            # Attempt to parse the response
            try:
                instrument_list = info['result']['list']
                if not instrument_list:
                    logger.critical("MEXC instruments list is empty. Setting min_qty=1.0")
                    self.min_qty = 1.0
                    return

                self.min_qty = float(instrument_list[0]['lotSizeFilter']['minOrderQty'])
                logger.info("Loaded min order qty", min_qty=self.min_qty)
            except KeyError as e:
                logger.critical(f"Unexpected MEXC response structure: {info}. Error: {e}. "
                                "Falling back to min_qty=1.0")
                self.min_qty = 1.0
            except (TypeError, ValueError) as e:
                logger.critical(f"Could not parse MEXC response: {info}. Error: {e}. "
                                "Falling back to min_qty=1.0")
                self.min_qty = 1.0

    async def check_open_trade(self) -> bool:
        """
        Check if there's an open position on MEXC.
        """
        try:
    ...
    async def stop(self):
        """
        Gracefully stop the trade service.
        """
        self.running = False
    ...
    def compute_adaptive_stop_loss_and_risk(self, current_price: float, atr_value: float,
                                            signal: str, market_regime: str, df_features: dict,
                                            base_risk_multiplier=1.5) -> (float, float):
        """
        Compute a dynamic stop loss and risk based on ATR.
        """
    ...
    async def update_open_trade_stops_task(self):
        """
        Periodically update trade stops for open positions based on new ATR, RSI, etc.
        """
        while self.running:
            if self.current_position is not None:
                try:
                    ...
                    boll_temp = ta.volatility.BollingerBands(df_15["close"], window=20, window_dev=2)
                    ...
                    stop_loss, take_profit = self.compute_sl_tp_dynamic(
                        current_price, df_15["atr"].iloc[-1], self.current_position,
                        market_regime, features, base_risk_multiplier=1.5
                    )
    ...
    async def set_trade_leverage(self, leverage: int):
        logger.info("Skipping set_leverage for MEXC (if not supported by API)")
        return
    ...
    async def execute_trade(self, side: str, qty: str,
        ...
        logger.info("Placing trade", side=side, size=qty,
    ...
    def _prepare_data_sequence(self, df: pd.DataFrame, feature_list: list, resample_period: str = '15min') -> np.ndarray:
        ...
        try:
            ...
            logger.warning("sklearn not installed? Please install for scaling features.")
            return None
        ...
        # If the ML service has PCA, we apply it here
    ...
    def predict_trend(self, recent_data: pd.DataFrame) -> str:
        """
        Wrapper to call MLService's trend model.
        """
        if self.trend_model:
            data_seq = self._prepare_data_sequence(recent_data,
                                                   self.actual_trend_cols or self.trend_feature_cols,
    ...
    def predict_signal(self, recent_data: pd.DataFrame) -> str:
        """
        Wrapper to call MLService's signal model (potentially ensemble).
        """
        if self.signal_models:
            data_seq = self._prepare_data_sequence(recent_data,
                                                   self.actual_signal_cols or self.signal_feature_cols,
                                                   resample_period="15min")
            if data_seq is None:
                return "Hold"
    ...
    async def run_trading_logic(self):
        """
        Main trading loop:
          1. Wait for ML models to be ready.
          2. Fetch recent candles.
          3. Compute signals.
          4. Execute trade if conditions match (with ATR-based sizing and stops).
          5. Sleep and repeat.
        """
        ...
        while self.running:
            try:
                ...
                stop_distance = abs(current_price - self.compute_adaptive_stop_loss_and_risk(
                    current_price, atr_value, signal, trend_prediction,
