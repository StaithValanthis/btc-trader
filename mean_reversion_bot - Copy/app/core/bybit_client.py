...
class BybitClient:
    """
    Async context manager for Bybit’s public WebSocket.
    Yields a connected `websockets` client.
    """
    def __init__(self):
    ...
    async def __aenter__(self):
        logger.info("Connecting WS", url=self.url)
...
class BybitMarketData:
    """
    Stateful market-data listener:
      - subscribes to publicTrade.<symbol>
      - writes trades into `market_data` table
      - auto-reconnects on idle or errors
    """
    def __init__(self):
        # pull your symbols list at startup
        self.symbols              = fetch_top_symbols()
        self.ws                   = None
        self.running              = False
    ...
    async def _connect_websocket(self):
        try:
            ...
            logger.debug("Subscription response", response=json.loads(resp))
            self.reconnect_attempts = 0
            logger.info("WebSocket connected")
